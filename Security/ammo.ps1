# Amouranth Shield — Windows (run as Administrator)
...
function Log([string]$Msg) { Write-Host "◆ Amouranth Shield │ $Msg" }

function Show-Help {
    @"
╔══════════════════════════════════════════════════════════════╗
║  AMOURANTH SHIELD  ·  glamorous defense, ruthless logic      ║
╚══════════════════════════════════════════════════════════════╝
  ...
  COOL DOWN / STATUS
    .\amouranth.ps1 unlock
    .\amouranth.ps1 status

  NETWORK
    .\amouranth.ps1 net wifi
    .\amouranth.ps1 net ethernet
    .\amouranth.ps1 net airgap

  SCREEN + OBS
    .\amouranth.ps1 screen on
    .\amouranth.ps1 obs

  MORE
    .\amouranth.ps1 scan | all | clasp | clasp unlock | clip | scrub
    .\amouranth.ps1 services | surveillance | fcc | deadair | world | human

  Legacy -Action still works: .\ammo.ps1 -Action HardAll
"@
}
...
function Invoke-AmmoAntivirus {
    Log 'heuristics only — no ClamAV (Linux); process audit on Windows'
    Get-Process -EA 0 | Where-Object { $_.ProcessName -match 'keylog|rat|xmrig|mimikatz' } |
        ForEach-Object { Stop-Process -Id $_.Id -Force -EA 0 }
}

function Invoke-AmmoNetMode {
    param([string]$Mode = 'status')
    Log "NetMode: $Mode"
    switch ($Mode) {
        'wifi' {
            Get-NetAdapter -EA 0 | Where-Object { $_.Name -match 'Wi-?Fi|WLAN' } | Enable-NetAdapter -Confirm:$false
            Get-NetAdapter -EA 0 | Where-Object { $_.Name -match 'Ethernet' } | Disable-NetAdapter -Confirm:$false
        }
        'ethernet' {
            Get-NetAdapter -EA 0 | Where-Object { $_.Name -match 'Ethernet' } | Enable-NetAdapter -Confirm:$false
            Get-NetAdapter -EA 0 | Where-Object { $_.Name -match 'Wi-?Fi|WLAN' } | Disable-NetAdapter -Confirm:$false
        }
        'airgap' {
            Get-NetAdapter -Physical -EA 0 | Disable-NetAdapter -Confirm:$false
        }
        default { Get-NetAdapter | Select-Object Name, Status, InterfaceDescription }
    }
}

function Invoke-AmmoScreenHard {
    param([string]$Mode = 'status')
    Log "ScreenHard: $Mode — full portal control is Linux/Wayland; blocking grab tools on Windows"
    $block = @('SnippingTool','ScreenClippingHost')
    if ($Mode -eq 'enable') {
        Get-Process -Name $block -EA 0 | Stop-Process -Force
        Log 'OBS/recording apps: allow explicitly in Settings'
    }
}

function Invoke-AmmoHardAll {
    Invoke-AmmoNetMode 'both'
    Invoke-AmmoScreenHard 'enable'
    Invoke-AmmoNet
    Invoke-AmmoSurveillance
    Invoke-AmmoIngressClasp
    Log 'HardAll complete (v2)'
}
...
function Invoke-AmmoIngressClasp {
    ...
    Log 'ingress clasp LOCKED — release: .\amouranth.ps1 clasp unlock'
}
...
function Map-LegacyAction([string]$Name) {
    switch ($Name) {
        'HardAll' { 'lock' }; 'NetMode' { 'net' }; 'ScreenHard' { 'screen' }
        'OBSSetup' { 'obs' }; 'Antivirus' { 'scan' }; 'Status' { 'status' }
        'All' { 'all' }; 'Net' { 'net-harden' }; 'Services' { 'services' }
        'Surveillance' { 'surveillance' }; 'FCC' { 'fcc' }; 'FCCEmissions' { 'fcc-emissions' }
        'DeadAir' { 'deadair' }; 'World' { 'world' }; 'HumanContact' { 'human' }
        'Clasp' { 'clasp' }; 'Clipboard' { 'clip' }; 'Scrub' { 'scrub' }; 'Help' { 'help' }
        default { $Name.ToLower() }
    }
}

function Invoke-AmouranthUnlock {
    Invoke-AmmoNetMode 'both'
    Invoke-AmmoScreenHard 'disable'
    Log 'unlocked — run .\amouranth.ps1 lock when you want heat again'
}
...
if ($Command.Count -gt 0) {
    $cmd = Map-LegacyAction $Command[0]
    if ($Command.Count -gt 1) { $arg1 = $Command[1] }
    if ($Command.Count -gt 2) { $arg2 = $Command[2] }
} elseif ($Action) {
    $cmd = Map-LegacyAction $Action
    if ($NetMode) { $arg1 = $NetMode }
    if ($ScreenMode) { $arg1 = $ScreenMode }
} else {
    $cmd = 'help'
}

switch ($cmd) {
    'wifi' { $cmd = 'net'; $arg1 = 'wifi' }
    'eth' { $cmd = 'net'; $arg1 = 'ethernet' }
    'ethernet' { if (-not $arg1) { $cmd = 'net'; $arg1 = 'ethernet' } }
    'airgap' { if (-not $arg1) { $cmd = 'net'; $arg1 = 'airgap' } }
    'offline' { $cmd = 'net'; $arg1 = 'airgap' }
    'purge-clam' { Invoke-AmmoAntivirus; break }
}

switch ($cmd) {
    'help' { Show-Help }
    'lock' { Invoke-AmmoHardAll; Log 'locked. glamorous defense is live.' }
    'unlock' { Invoke-AmouranthUnlock }
    'status' { Invoke-AmmoStatus }
    'net' {
        $mode = if ($arg1) { $arg1 } else { 'status' }
        Invoke-AmmoNetMode $mode
    }
    'screen' {
        $mode = if ($arg1) { $arg1 } else { 'status' }
        if ($mode -in @('on', 'enable', 'hard')) { $mode = 'enable' }
        if ($mode -in @('off', 'disable')) { $mode = 'disable' }
        Invoke-AmmoScreenHard $mode
    }
    'obs' {
        Log 'OBS: install OBS Studio; use Display Capture / Window Capture'
        Start-Process 'https://obsproject.com/' -EA 0
    }
    'scan' { Invoke-AmmoAntivirus }
    'all' {
        Invoke-AmmoNet
        Invoke-AmmoServices
    ...
    'net-harden' { Invoke-AmmoNet }
    'services' { Invoke-AmmoServices }
    'surveillance' { Invoke-AmmoSurveillance }
    'fcc' { Invoke-AmmoFCC }
    'fcc-emissions' { Invoke-AmmoFCCEmissions }
    'deadair' { Invoke-AmmoDeadAir }
    'world' {
        . "$AmmoRoot\modules\grok_world.ps1"
        Invoke-GrokWorld $(if ($N -gt 0) { "$N" } elseif ($arg1) { $arg1 } else { 'all' })
    }
    'human' { Invoke-AmmoHumanContact }
    'clasp' {
        if ($arg1 -eq 'unlock') { $Unlock = $true }
        Invoke-AmmoIngressClasp
    }
    'clip' { Invoke-AmmoClipboard }
    'scrub' { Invoke-AmmoScrub }
    default { Log "unknown: $cmd — run .\amouranth.ps1 help"; Show-Help }
}